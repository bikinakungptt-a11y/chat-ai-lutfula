...
export default async function handler(req, res) {
  ...
  try {
    ...
    const upstream = await fetch(endpoint, { headers: { 'X-Api-Key': apiKey } });
    ...
    if (!upstream.ok) {
      return json(res, upstream.status, { ok: false, error: 'API Ninjas isworkingday failed.', status: upstream.status, details: data });
    }

    json(res, 200, { ok: true, date, country, isWorkingDay: Boolean(data?.is_working_day), raw: data });
  } catch (error) {
